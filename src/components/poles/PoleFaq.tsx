...
import { poleCopy, poleFaqs } from "@/lib/pole-content";

export function PoleFaq() {
          ...
          {poleFaqs.map((faq, i) => {
            ...
            return (
              <div key={faq.q} className="border-b border-[#D6D2CA]">
                <button type="button" onClick={() => setOpen(isOpen ? -1 : i)} aria-expanded={isOpen} className="flex w-full items-center justify-between gap-6 py-6 text-left">
